...
- **terminal echo control** — a new `Engine(set_echo=fn)` seam hook plus `mon.tty.echo` (the terminal echo mode, default on). Assigning `mon.tty.echo` drives `set_echo`, so a program toggling echo (a TOPS-10 `SETSTS`/`INIT` TTY-mode change — off for a raw single-key read, back on for line input) actually changes the terminal, not a flag nobody reads. The `pyfortran10`/`pyf66` CLI wires this to the real tty — it flips the termios `ECHO` bit (restored on exit) — so echo control works running a deck straight from the command line, not only under an embedding harness. Off a terminal (piped/redirected, or headless) the state is simply recorded.
