...
Current gaps (14 files fail to parse under the F77 front-end -- mostly the F77 I/O set):
...
Of the 126 that run: 1259 sub-tests PASS, 155 ERRORS, 44 print-and-eyeball (no summary).
...
constants, the widthless A descriptor, and list-directed I/O (split into its own dialect flag).
"""
...
def test_f77_conformance_baseline():
    ...
    assert R["n_run"] == 126
    assert R["n_gap"] == 14
    assert R["total_pass"] == 1259
    assert R["total_err"] == 155
    assert len(R["nosummary"]) == 44
